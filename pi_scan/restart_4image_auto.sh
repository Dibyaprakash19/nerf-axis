...
echo "Restoring 4-image automated scanner on $PI_USER@$HOST"
ssh -t "$PI_USER@$HOST" "
    set -e
    echo 'Stopping any live preview/scanner process...'
    pkill -f '[p]ython3 tft_cam.py' 2>/dev/null || true
    pkill -f '[p]ython3 -m nerf_scan.main_16' 2>/dev/null || true

    echo 'Pointing systemd back to the 4-image scanner...'
    ...
    echo 'Reloading and enabling boot automation...'
...
echo "4-image automated scanner is restored and enabled for reboot."
