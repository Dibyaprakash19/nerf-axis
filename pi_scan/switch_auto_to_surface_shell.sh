...
echo "Switching default automation to 4-view surface-shell scanner on $PI_USER@$HOST"
ssh -t "$PI_USER@$HOST" "
    set -e
    pkill -f '[p]ython3 tft_cam.py' 2>/dev/null || true
    pkill -f '[p]ython3 -m nerf_scan.main' 2>/dev/null || true
    pkill -f '[p]ython3 -m nerf_scan.main_4view_surface_shell' 2>/dev/null || true

    sudo sed -i 's|^ExecStart=.*|ExecStart=/usr/bin/python3 -m nerf_scan.main_4view_surface_shell|g' /etc/systemd/system/nerf_scan.service
...
echo "Default automation is now 4-view surface-shell."
