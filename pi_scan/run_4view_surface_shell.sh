...
RESULTS_DIR="$HERE/results/surface_shell_latest"
...
cleanup() {
    ...
    mkdir -p "$RESULTS_DIR"
    rsync -az "$PI_USER@$HOST:$PI_DIR/data/surface_shell/" "$RESULTS_DIR/" >/dev/null 2>&1 || true
    echo "Local result folder: $RESULTS_DIR"
}

trap cleanup EXIT INT TERM

echo "Running 4-view MiDaS surface-shell mesh."
echo "Each view becomes a separate relief surface around the object."
...
ssh -t "$PI_USER@$HOST" "
    cd $PI_DIR
    . venv/bin/activate
    exec python3 -m nerf_scan.main_4view_surface_shell
"
