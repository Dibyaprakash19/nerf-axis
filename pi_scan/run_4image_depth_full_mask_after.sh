...
RESULTS_DIR="$HERE/results/depth_full_mask_after_latest"
...
cleanup() {
    ...
    ssh "$PI_USER@$HOST" "
        pkill -f '[p]ython3 -m nerf_scan.main_4_depth_full_mask_after' 2>/dev/null || true
        sudo systemctl start nerf_scan || true
    " || true

    mkdir -p "$RESULTS_DIR"
    rsync -az "$PI_USER@$HOST:$PI_DIR/data/depth_full_mask_after/" "$RESULTS_DIR/" >/dev/null 2>&1 || true
    echo "Local result folder: $RESULTS_DIR"
}

trap cleanup EXIT INT TERM

echo "Running 4-view test: MiDaS sees full image, fusion sees masked object."
...
ssh -t "$PI_USER@$HOST" "
    cd $PI_DIR
    . venv/bin/activate
    exec python3 -m nerf_scan.main_4_depth_full_mask_after
"
